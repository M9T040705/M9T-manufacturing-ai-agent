...
**访问地址：** http://47.98.122.252:8503

**服务器配置：** 阿里云 ECS · CentOS 7 · 2核2GB

### 演示账号

| 角色 | 账号 | 密码 | 能看什么 |
|------|------|------|----------|
| 🥇 管理员（最全面） | `admin` | `admin123` | 全部权限（用户管理/模型配置/运维 + 所有业务场景） |
| 厂长 | `boss` | `boss123` | 全部6个场景 + 审批操作 |
| 财务 | `finance` | `fin123` | 仅采购对账 |
| 销售 | `sales` | `sales123` | 仅交期答复 |
| 生产 | `prod` | `prod123` | 齐套检查 + 管理日报 + 设备维保 |
| 质量 | `qc` | `qc123` | 仅质量追溯 |
